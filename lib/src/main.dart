...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _incrementCounter() {
    setState(() async {
      ...
      AssetSource fileSource = AssetSource('test_sound.mp3');
      player.play(fileSource);

      final bundle = DefaultAssetBundle.of(context);
      final bytes = await bundle.load('assets/sounds/test/test_sound.mp3');
      final tempDir = await getTemporaryDirectory();
      final file = await File('${tempDir.path}/test_sound.mp3').create();
      await file.writeAsBytes(bytes.buffer.asUint8List());
      
      return Share.shareFiles(['${tempDir.path}/test_sound.mp3'], mimeTypes: ['audio/wav']);
    });
  }
